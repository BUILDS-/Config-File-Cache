...
./.cabal: 
config - importantly set global install and sudo

./.canto:
conf.py - includes feed handler and urls
...
gmail
hotmail
script.sh - hands off mail intelligently

./.mpdscribble:
mpdscribble.conf - username, password, and socket address for last.fm submission

./.ncmpcpp:
config - major changes are for socket stuff

./.sup:
config.yaml - general configuration
sources.yaml - sending and receiving

./.xmonad:
xmonad.hs - pile of derp.  I like having shortcuts I can type with one hand, so this is in transition toward that.
